...
<body>
    ...
    <?php
        ...
        $numUpd = $_GET['num'];
        $query = "SELECT * FROM works WHERE num='$numUpd'";
        $result = mysqli_query($connect,$query);
        $row = mysqli_fetch_array($result);
    ?>
    <form action="update-ok.php?num=<?=$row['num']?>" method="post"
        enctype="multipart/form-data">
        <table>
            <caption>Portfolio update</caption>
            <tr>
                <th>title</th>
            ...
            <tr>
                <th>thumbnail</th>
                <td><input type="file" name="thumbnail"><? echo $row['thumbnail']?></td>
            </tr>
            <tr>
                <th>projectIntro</th>
                <td><textarea name="projectIntro" >
                    <? echo nl2br($row['projectIntro'])?></textarea></td>
            </tr>
            <tr>
                <th>contents</th>
                <td><textarea name="contents" id="ir1"><? echo $row['contents']?></textarea></td>
            </tr>
            <tr>
                <th>dateStart</th>
                <td><input type="date" name="dateStart" value="<? echo $row['dateStart']?>"></td>
            </tr>
            <tr>
                <th>dateEnd</th>
                <td><input type="date" name="dateEnd" value="<? echo $row['dateEnd']?>"></td>
            </tr>
            <tr>
                <th>action</th>
                <td>
